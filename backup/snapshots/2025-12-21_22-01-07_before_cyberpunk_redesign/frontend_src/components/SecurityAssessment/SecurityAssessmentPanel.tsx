      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-700">Antivirus</h3>
          ...
          {assessment.antivirus.installed ? (
            ...
            <p className="text-sm text-danger-600 font-medium">Nessun antivirus rilevato</p>
          )}
        </div>
        ...
        <div className="p-4 bg-gray-50 rounded-lg">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-gray-700">Firewall</h3>
          ...
          {assessment.firewall.enabled ? (
            ...
            <p className="text-sm text-danger-600 font-medium">Firewall disabilitato</p>
          )}
        </div>
      </div>
              ...
              <div key={i} className="p-3 border border-gray-200 rounded-lg">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex-1">
                    ...
                    {vuln.cveId && (
                      <p className="text-xs text-gray-500 mb-1">CVE: {vuln.cveId}</p>
                    )}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {assessment.openPorts.slice(0, 8).map((port, i) => (
              <div key={i} className="p-2 bg-gray-50 rounded text-center">
                <p className="text-sm font-medium text-gray-800">{port.port}</p>
                <p className="text-xs text-gray-600">{port.protocol}</p>
                {port.service && (
                  <p className="text-xs text-gray-500">{port.service}</p>
                )}
              </div>
            ))}
          </div>
