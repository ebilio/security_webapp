...
async function detectWindowsAntivirus(): Promise<AntivirusStatus> {
  ...
  try {
    ...
    if (avList.length > 0) {
      ...
      // DD = Definition status (00-0F=updated, 10+=out-of-date)
      ...
      const decodeProductState = (state: number) => {
        ...
        // Formato: 0xDDSSPP
        // DD (primi 2 char) = Definition status (00-0F=updated, 10+=out-of-date)
        ...
        // Definitions are updated if byte value < 0x10 (16 decimal)
        // Common values: 0x00,0x01,0x04,0x06 = updated, 0x10+ = out-of-date
        const defByte = parseInt(defStatus, 16);
        const updated = defByte < 0x10;
