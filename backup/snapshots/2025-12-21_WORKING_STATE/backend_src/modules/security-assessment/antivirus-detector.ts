...
async function detectWindowsAntivirus(): Promise<AntivirusStatus> {
  ...
  try {
    ...
    if (avList.length > 0) {
      ...
      // OTTIMIZZAZIONE: limita a massimo 2 antivirus per evitare troppi check
      ...
      for (let i = 0; i < Math.min(avList.length, maxAVToCheck); i++) {
        ...
        try {
          ...
          console.log(`Error checking ${av.displayName}, skipping:`, error);
          continue;
        }
      }

      if (avList.length > maxAVToCheck) {
        console.log(`Skipped ${avList.length - maxAVToCheck} antivirus products to avoid performance issues`);
      }
